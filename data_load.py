...
def load_data(mode="train"):
    ...
    if mode in ("train", "eval"):
        ...
        if mode=="train":
            ...
            #print('inside else in dataload load_data');
            lines = lines[:1]

        for line in lines:
            #print("split line from file lines", line.strip().split(","))
            fname, _, text = line.strip().split(",")

            fpath = os.path.join(hp.data, "clips/", fname + ".wav")
            fpaths.append(fpath)
        ...
        # Parse
        print("READING CUSTOM SENTENCES FILES LINE")
